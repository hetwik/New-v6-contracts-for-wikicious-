/**
 * Wikicious V6 — Mainnet Contract Verification Script
 * 
...
const NO_OWNER = new Set(["WikiMultisigGuard", "WikiTimelockController", "WikiPushNotification"]);
...
async function checkContract(name, address) {
  const result = { name, address, bytecode: false, owner: null, pendingOwner: null, issues: [] };
...
async function main() {
  ...
  for (let i = 0; i < names.length; i += BATCH) {
    ...
    for (const s of settled) {
      ...
      else results.push({ name: "?", issues: [s.reason?.message || "unknown error"] });
    }
    process.stdout.write(`  Checked ${Math.min(i + BATCH, names.length)}/${names.length}...\r`);
  }
  ...
  // Save report
  const fs = require("fs");
  const report = { timestamp: new Date().toISOString(), safe: SAFE, total: results.length, clean: allGood.length, issues: issues.length, details: results };
