...
function normalizeAddress(value, label, { allowZero = true } = {}) {
  ...
  raw = raw.replace(/^['"]|['"]$/g, "");
  const hadPrefix = raw.startsWith("0x");
  const sourceHex = hadPrefix ? raw.slice(2) : raw;
  const hexOnly = sourceHex.replace(/[^0-9a-fA-F]/g, "");

  let normalizedHex = hexOnly;
  if (normalizedHex.length === 39) {
    normalizedHex = `0${normalizedHex}`;
    console.warn(`⚠️  ${label} had 39 hex chars; auto-padded leading 0.`);
  }
  if (normalizedHex.length > 40) {
    console.warn(`⚠️  ${label} had extra characters; using first 40 hex chars.`);
    normalizedHex = normalizedHex.slice(0, 40);
  }
  if (normalizedHex.length === 40) {
    raw = `0x${normalizedHex}`;
  } else if (!hadPrefix && /^[0-9a-fA-F]{40}$/.test(raw)) {
    raw = `0x${raw}`;
  }
