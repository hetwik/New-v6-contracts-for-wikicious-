...
async function d(name, ...args) {
  ...
  if (args.length > expectedArgs) {
    ...
    const missingInputs = (F.interface.deploy?.inputs || []).slice(args.length);
    const defaults = missingInputs.map((input) => {
      const t = input.type || "";
      if (t.includes("[]")) return [];
      if (t === "address") return DEFAULT_DEPLOYER_FOR_ARGS;
      if (t.startsWith("uint") || t.startsWith("int")) return 0;
      if (t === "bool") return false;
      if (t.startsWith("bytes")) return t === "bytes" ? "0x" : ethers.ZeroHash;
      if (t === "string") return "";
      return 0;
    });
