/**
 * Wikicious V6 — Arbiscan Source Code Verification (Etherscan V2 API)
 *
 * Verifies all 133 contracts on Arbiscan using hardhat verify.
 * Reads constructor args from deployment JSON automatically.
 *
 * Run: node verify-etherscan-v2.js
 * Requires: ALCHEMY_ARBITRUM_URL + ETHERSCAN_API_KEY in .env
 */
...
// ─── Main ──────────────────────────────────────────────────────────────────
async function main() {
  console.log("🔍  Wikicious V6 — Arbiscan Source Verification (Etherscan V2 API)");
  console.log(`    Verifying ${CONTRACTS.length} contracts on Arbitrum One...\n`);

  const results = { verified: [], already_verified: [], failed: [] };
  const DELAY = 1500; // ms between contracts — free tier: 5 req/sec
